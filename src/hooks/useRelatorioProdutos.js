// src/hooks/useRelatorioProdutos.js
...
/**
 * Hook personalizado para buscar os dados do relatório de produtos.
 * @returns {{reportData: Array, loading: boolean, fetchReport: (dates: {start: string, end: string}) => Promise<void>}}
 */
export const useRelatorioProdutos = (initialDates) => {
  ...
  const fetchReport = useCallback(async (dates) => {
    try {
      ...
      notify.error(err.message || 'Não foi possível gerar o relatório de produtos.');
    } finally {
      setLoading(false);
    }
  }, [notify]);
