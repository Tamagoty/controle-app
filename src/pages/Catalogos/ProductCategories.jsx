// src/pages/Catalogos/ProductCategories.jsx
...
const ProductCategories = () => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await supabase.from('product_categories').update({ is_active: newStatus }).eq('id', id);
  ...
  return (
    <div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingCategory ? 'Editar Categoria' : 'Nova Categoria'}>
        <ProductCategoryForm onSuccess={handleSuccess} categoryToEdit={editingCategory} />
      </Modal>
    </div>
  );
};
