-- =================================================================
-- SCRIPT 2: FUNÇÕES E AUTOMAÇÕES (VERSÃO COMPLETA)
...
BEGIN
    INSERT INTO public.entities (name, email, phone, document_number, address, entity_type) VALUES (name_param, email_param, phone_param, document_number_param, address_param, entity_type_param) RETURNING id INTO new_entity_id;
...
DECLARE
    role_item TEXT;
BEGIN
    UPDATE public.entities SET name = name_param, email = email_param, phone = phone_param, document_number = document_number_param, address = address_param, entity_type = entity_type_param WHERE id = entity_id_param;
...
LANGUAGE sql STABLE AS $$ SELECT e.id, e.name, e.email, e.phone, e.document_number, e.address, e.entity_type, e.is_active, string_agg(er.role, ', ') as roles FROM public.entities e LEFT JOIN public.entity_roles er ON e.id = er.entity_id GROUP BY e.id, e.name, e.email, e.phone, e.document_number, e.address, e.entity_type, e.is_active ORDER BY e.name ASC; $$;
...
RETURNS JSON LANGUAGE plpgsql AS $$
DECLARE
    report_data JSON; transactions_data JSONB := '[]'::JSONB; rec RECORD; total_inflow_calc NUMERIC := 0; total_outflow_calc NUMERIC := 0; net_profit_calc NUMERIC;
BEGIN
    FOR rec IN SELECT 'Receita de Venda' as description, sp.amount_paid, sp.payment_date FROM public.sale_payments sp WHERE sp.payment_date BETWEEN p_start_date AND p_end_date LOOP transactions_data := transactions_data || jsonb_build_object('date', rec.payment_date, 'type', 'Receita', 'description', rec.description, 'amount', rec.amount_paid); total_inflow_calc := total_inflow_calc + rec.amount_paid; END LOOP;
    FOR rec IN SELECT 'Pagamento a Fornecedor' as description, pp.amount_paid, pp.payment_date FROM public.purchase_payments pp WHERE pp.payment_date BETWEEN p_start_date AND p_end_date LOOP transactions_data := transactions_data || jsonb_build_object('date', rec.payment_date, 'type', 'Despesa', 'description', rec.description, 'amount', rec.amount_paid); total_outflow_calc := total_outflow_calc + rec.amount_paid; END LOOP;
    FOR rec IN SELECT ge.description, ep.amount_paid, ep.payment_date FROM public.expense_payments ep JOIN public.general_expenses ge ON ep.expense_id = ge.id WHERE ep.payment_date BETWEEN p_start_date AND p_end_date LOOP transactions_data := transactions_data || jsonb_build_object('date', rec.payment_date, 'type', 'Despesa', 'description', rec.description, 'amount', rec.amount_paid); total_outflow_calc := total_outflow_calc + rec.amount_paid; END LOOP;
    FOR rec IN SELECT 'Pagamento de Comissão' as description, cp.amount_paid, cp.payment_date FROM public.commission_payments cp WHERE cp.payment_date BETWEEN p_start_date AND p_end_date LOOP transactions_data := transactions_data || jsonb_build_object('date', rec.payment_date, 'type', 'Despesa', 'description', rec.description, 'amount', rec.amount_paid); total_outflow_calc := total_outflow_calc + rec.amount_paid; END LOOP;
...
CREATE OR REPLACE FUNCTION public.get_entities_without_partner_role()
RETURNS TABLE(id UUID, name TEXT)
LANGUAGE sql STABLE AS $$ SELECT e.id, e.name FROM public.entities e WHERE e.entity_type = 'Pessoa' AND NOT EXISTS ( SELECT 1 FROM public.entity_roles er WHERE er.entity_id = e.id AND er.role = 'Sócio' ); $$;
...
UNION ALL
SELECT pp.payment_date as transaction_date, 'Despesa de Compra' as transaction_type, 'Compra de ' || s.name as description, pp.amount_paid as amount, p.cost_center_id, cc.name as cost_center_name, p.supplier_id as entity_id, s.name as entity_name, 'Compras' as category_name FROM public.purchase_payments pp JOIN public.purchases p ON pp.purchase_id = p.id JOIN public.entities s ON p.supplier_id = s.id JOIN public.cost_centers cc ON p.cost_center_id = cc.id WHERE pp.payment_date BETWEEN p_start_date AND p_end_date
UNION ALL
SELECT ep.payment_date as transaction_date, 'Despesa Geral' as transaction_type, ge.description, ep.amount_paid as amount, ge.cost_center_id, cc.name as cost_center_name, ge.employee_id as entity_id, e.name as entity_name, ec.name as category_name FROM public.expense_payments ep JOIN public.general_expenses ge ON ep.expense_id = ge.id JOIN public.cost_centers cc ON ge.cost_center_id = cc.id LEFT JOIN public.entities e ON ge.employee_id = e.id JOIN public.expense_categories ec ON ge.category_id = ec.id WHERE ep.payment_date BETWEEN p_start_date AND p_end_date;
$$;
...
LANGUAGE sql STABLE AS $$ SELECT p.id as product_id, p.name as product_name, SUM(si.quantity) as total_quantity_sold, SUM(si.total_price) as total_revenue, AVG(si.unit_price) as average_price, COUNT(DISTINCT si.sale_id) as number_of_sales FROM public.sale_items si JOIN public.products p ON si.product_id = p.id JOIN public.sales s ON si.sale_id = s.id WHERE s.sale_date BETWEEN p_start_date AND p_end_date GROUP BY p.id, p.name ORDER BY total_revenue DESC; $$;
...
DECLARE top_clients_data JSONB; sales_history_data JSONB;
BEGIN
    SELECT jsonb_agg(client_sales) INTO top_clients_data FROM ( SELECT e.name as client_name, SUM(si.total_price) as total_revenue FROM public.sale_items si JOIN public.sales s ON si.sale_id = s.id JOIN public.entities e ON s.client_id = e.id WHERE si.product_id = p_product_id AND s.sale_date BETWEEN p_start_date AND p_end_date GROUP BY e.name ORDER BY total_revenue DESC LIMIT 5 ) as client_sales;
    SELECT jsonb_agg(sale_details) INTO sales_history_data FROM ( SELECT s.id as sale_id, s.sale_date, e.name as client_name, si.quantity, si.total_price FROM public.sale_items si JOIN public.sales s ON si.sale_id = s.id JOIN public.entities e ON s.client_id = e.id WHERE si.product_id = p_product_id AND s.sale_date BETWEEN p_start_date AND p_end_date ORDER BY s.sale_date DESC ) as sale_details;
...
LANGUAGE sql STABLE AS $$ SELECT e.id as client_id, e.name as client_name, SUM(s.total_amount) as total_revenue, COUNT(s.id) as number_of_sales FROM public.sales s JOIN public.entities e ON s.client_id = e.id WHERE s.sale_date BETWEEN p_start_date AND p_end_date GROUP BY e.id, e.name ORDER BY total_revenue DESC; $$;
...
LANGUAGE sql STABLE AS $$ SELECT s.id as sale_id, s.sale_date, s.total_amount, string_agg(p.name, ', ') as products_summary FROM public.sales s JOIN public.sale_items si ON s.id = si.sale_id JOIN public.products p ON si.product_id = p.id WHERE s.client_id = p_client_id AND s.sale_date BETWEEN p_start_date AND p_end_date GROUP BY s.id, s.sale_date, s.total_amount ORDER BY s.sale_date DESC; $$;
